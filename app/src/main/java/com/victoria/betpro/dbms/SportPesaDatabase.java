...
import com.victoria.betpro.Bet;
import com.victoria.betpro.BetsData;

import java.util.ArrayList;

public class SportPesaDatabase extends SQLiteOpenHelper {

    public SportPesaDatabase(Context context)
    {
        super(context, "bets.db", null, 2);//modify here
    }
    @Override
     ...
     */
    public void save_bet(String bet_id, int bet_amount, String month,String company, long time_int, String unique_id)//Modify here to suit what you want to save
    {
    ...
    public void save_win(String bet_id, int amount_won, String month,String company, long time_int, String unique_id)
    {
        SQLiteDatabase db=this.getWritableDatabase();
        ContentValues values=new ContentValues();
        values.put("bet_id", bet_id);//modify here
    ...
    public ArrayList<Bet> retrieveBets()
    {
        ...
        if(cursor.moveToFirst())
        {
            do
            {
                ...
                String bet_id=cursor.getString(1);
                String bet_amount=cursor.getString(2);
                String month=cursor.getString(3);
                String company=cursor.getString(4);
                String time_int=cursor.getString(5);
                String unique_id=cursor.getString(6);
                Bet b=new Bet(id,bet_id,bet_amount,month,company,time_int,unique_id);
                bets.add(b);
    ...
    public BetsData getBets(long from,long to){
        SQLiteDatabase db=this.getReadableDatabase();
        ArrayList<Entry> win_entries=new ArrayList<>();
        ArrayList<Entry> bet_entries=new ArrayList<>();
        ArrayList<String> months=new ArrayList<>();

        String sql = "SELECT month, SUM(bet_amount) FROM bets WHERE time_int>="+from+" AND time_int<="+to+" GROUP BY month order by _id desc";
        Log.d("BET_FILTER", sql);
        Cursor cursor = db.rawQuery(sql,null);
        int x=0;
        ...
        if(cursor.moveToFirst()){
            while(cursor.moveToNext())
            {
                ...
                //Entry win=new Entry((float)amount_won, x);
                Entry bet=new Entry((float)amount_bet,x);
                Log.d("BET_FILTER_WIN_AMOUNT", " : "+amount_bet);
                //win_entries.add(win);
                bet_entries.add(bet);
                months.add(month);
                x++;
        ...
        String sql2 = "SELECT SUM(amount_won) FROM wins WHERE time_int>="+from+" AND time_int<="+to+" GROUP BY month order by _id desc";
        Log.d("BET_FILTER", sql2);
        Cursor cursor2 = db.rawQuery(sql2,null);
        Log.d("WIN_COUNT",""+cursor2.getCount());
        int y=0;
        if(cursor2.moveToFirst())
        {
            while(cursor2.moveToNext())
            {
                //String month= cursor2.getString(0);
                int amount_won =cursor2.getInt(0);
                Log.d("BET_FILTER_WIN_AMOUNT", " : "+amount_won);
                Entry win=new Entry((float)amount_won, y);
                win_entries.add(win);
                y++;
            }
    ...
    public BetsData getBarData(long from,long to){
        SQLiteDatabase db=this.getReadableDatabase();
        ArrayList<BarEntry> win_entries=new ArrayList<>();
        ArrayList<BarEntry> bet_entries=new ArrayList<>();
        ArrayList<String> months=new ArrayList<>();
        ...
        BetsData data=new BetsData(win_entries,bet_entries,"Bar Data",months);
        return data;
    }
}
