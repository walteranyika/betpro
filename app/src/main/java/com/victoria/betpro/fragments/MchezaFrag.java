 ...
 */
public class MchezaFrag extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        db=new MchezaDatabase(getActivity());
        btnGraphs.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (db.count()>0){
                    Intent x=new Intent(getActivity(),GraphsActivity.class);
                    x.putExtra("company","mcheza");
                    startActivity(x);
                }else{
                    Snackbar.make(rootView, "You have no record for Mcheza Bets to display on the graph", Snackbar.LENGTH_LONG)
    ...
    public void get_db_values(MchezaDatabase db) {
