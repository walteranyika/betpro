 ...
 */
public class BetInFrag extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,   Bundle savedInstanceState) {
        ...
        db=new BetInDatabase(getActivity());
        btnGraphs.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (db.count()>0){
                    Intent x=new Intent(getActivity(),GraphsActivity.class);
                    x.putExtra("company","betin");
                    startActivity(x);
                }else {
                    Snackbar.make(rootView, "You have no records for BetIn Bets to display on the graph", Snackbar.LENGTH_LONG).setAction("Action", null).show();
                }
            }
        });
        get_db_values(db);
    ...
    public void get_db_values(BetInDatabase db) {
